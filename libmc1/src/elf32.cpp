...
extern "C" bool elf32_load(const char* file_name, uint32_t* entry_address) {
  ...
  for (unsigned i = 0U; i < elf_header.e_shnum; ++i) {
    ...
    // Note: .bss sections are marked with sh_type=SHT_NOBITS. These need to be cleared, but that is
    // done as part of the crt0 startup code. We don't want to do that here since that could
    // potentially interfere with our code (in case there are overlaps between our section
    // allocation and the section allocation of the loaded executable, e.g. if our .bss section is
    // reused by a loaded program that takes over control and never returns).
  }
